...
#include "infcoeff.h"
#include "safetan.h"
#include <stdio.h> /* PRL added for debugging */
...
#endif
{
	...
	double magy2    = sqrt(y2*y2); /* PRL CHANGE TO CHECK FOR VERY SMALL y2 */
    double termPRL1    = 0.0; /* PRL change to avoid "expression too complicated" error on PC lcc compiler */
    double termPRL2    = 0.0; /* PRL change to avoid "expression too complicated" error on PC lcc compiler */
    double termPRL3    = 0.0; /* PRL change to avoid "expression too complicated" error on PC lcc compiler */
    double termPRL4    = 0.0; /* PRL change to avoid "expression too complicated" error on PC lcc compiler */
    double termPRL5    = 0.0; /* PRL change to avoid "expression too complicated" error on PC lcc compiler */
    double termPRL6    = 0.0; /* PRL change to avoid "expression too complicated" error on PC lcc compiler */
	...
	*/
	...
	------------------------------------*/
	/* PRL CHANGE */
	if (magy2 < 1.0e-10) f = fb = 0.0;
/*	      f  = 0.0;
	      fb = 0.0;
	}*/
	/* END PRL CHANGE */
	/*if (print_elt != 0) {
	      fprintf(stdout," y: %lg %lg %lg,  sinb:  %lg, cosb: %lg, cotb: %lg\n", y1,y2,y3,sinb,cosb,cotb);
	      fprintf(stdout," y3b: %lg, z1: %lg, z3: %lg, r: %lg, z1b: %lg, z3b: %lg, rb: %lg, f: %lg, fb: %lg\n", y3b,z1,z3,r,z1b,z3b,rb,f,fb);
	      }
	*/
	displ_ic[0][0] = 
		-y1*y2*(1/(r*(r-y3))) - 
	...
	displ_ic_ft[2][2] = 2*(1-pr)*(f);
	     	/* PRL TEST PRINT */
	/*if (print_elt != 0) {
     fprintf(stdout," displ_ic:      %lg %lg %lg\n",displ_ic[0][0],displ_ic[0][1],displ_ic[0][2]);
     fprintf(stdout," displ_ic:      %lg %lg %lg\n",displ_ic[1][0],displ_ic[1][1],displ_ic[1][2]);
     fprintf(stdout," displ_ic:      %lg %lg %lg\n",displ_ic[2][0],displ_ic[2][1],displ_ic[2][2]);
     fprintf(stdout," displ_ic_ft:   %lg %lg %lg\n",displ_ic_ft[0][0],displ_ic_ft[0][1],displ_ic_ft[0][2]);
     fprintf(stdout," displ_ic_ft:   %lg %lg %lg\n",displ_ic_ft[1][0],displ_ic_ft[1][1],displ_ic_ft[1][2]);
     fprintf(stdout," displ_ic_ft:   %lg %lg %lg\n",displ_ic_ft[2][0],displ_ic_ft[2][1],displ_ic_ft[2][2]);
	}
    */
	if (half_space) {
		...
		displ_ic[0][1] += 2.0*(termPRL1+termPRL2+termPRL3+termPRL4+termPRL5);
/*		displ_ic[0][1] += 2.0*(
			/* term1 */
/*			(1-2*pr)*((2*(1-pr)*(cotb*cotb)-pr)*log(rb+y3b) -
			(2*(1-pr)*(cotb*cotb)+1-2*pr)*cosb*log(rb+z3b)) -
...
/*			(y3b-a)/(rb+z3b)*((cosb*cosb) - 
        ...
        termPRL6=
			...
			(rb*cosb+y3b)/(rb+z3b) *
			(1+a/(rb*cosb))));
        displ_ic[1][0] += 2.0*(termPRL1+termPRL2+termPRL3+termPRL4+termPRL5+termPRL6);
/*		displ_ic[1][0] += 2.0*( */
			/* term1 */
/*			(1-2*pr)*((2*(1-pr)*(cotb*cotb)+pr)*log(rb+y3b) -
			(2*(1-pr)*(cotb*cotb)+1)*cosb*log(rb+z3b)) +
...
*/			
        ...
        displ_ic[1][2] += 2.0*(termPRL1+termPRL2+termPRL3+termPRL4+termPRL5);
/*		displ_ic[1][2] += 2.0*(
*/			/* term1 */
/*			-2*(1-pr)*(1-2*pr)*cotb * 
			(log(rb+y3b)-cosb*log(rb+z3b)) -
...
*/			
        ...
        displ_ic[2][1] += 2.0*(termPRL1+termPRL2+termPRL3);
/*		displ_ic[2][1] += 2.0*(
*/			/* term1 */
/*			(1-2*pr)*(-sinb*log(rb+z3b) - y1/(rb+y3b)*(1+a/rb) +
			z1b/(rb+z3b)*(cosb+a/rb)) +
...
#endif
{
	...
	if (half_space) {
		...
		/* dg_ic[0][0][1] += 2.*( */
        termPRL1=
            (pr3*(-((y1*(pr + aQrb))/ rbPy3b) + cotb*(1. - 2.*pr - aQrb)))/ rbPy3b + 
        ...
        termPRL2=
			...
			(a*cotb*y3bMa)/rbP3;
        termPRL3=
            (y3bMa*((cosb*(pr2*cosb*rbTsinbMy1 + cotb*(cosb*pr3 - aQrb)*
			rbTcosbPy3b))/ rbPz3b - (a*cosb*cotb*y3b)/rbP2))/ (rbPz3b* rb) + 
			(y3bMa*(-(cotb*pr3) + (y1*(2.*pr + aQrb))/ rbPy3b + 
			(a*y1)/rbP2))/ (rbPy3b*rb);
        termPRL4=
            (y2*y3bMa*((cosb*(2.*cosb*sinb*(1. - pr)* DrbDy2 + cosb*cotb*
			(cosb*pr3 - aQrb)* DrbDy2 + (a*cotb*rbTcosbPy3b* DrbDy2)/
			rbP2))/ rbPz3b - (cosb*(pr2*cosb*rbTsinbMy1 + cotb*(cosb*pr3 - aQrb)*
			...
			rbPy3bP2;
        termPRL5=
            -(cosb*cotb*y2*pr3*(cosb + aQrb)* DrbDy2)/
			...
			(rbPz3b* rbP2);
        termPRL6=
            -(y2*y3bMa* (-(cotb*pr3) + (y1*(2.*pr + aQrb))/
        ...
        dg_ic[0][0][1] += 2.*(termPRL1+termPRL2+termPRL3+termPRL4+termPRL5+termPRL6);
/*		dg_ic[0][0][1] += 
            2.*((pr3*(-((y1*(pr + aQrb))/ rbPy3b) + cotb*(1. - 2.*pr - aQrb)))/ rbPy3b + 
			(cosb*cotb*pr3*(cosb + aQrb))/ rbPz3b - 
			2.*cotbP2*pr3*(1. - pr)* (DrbDy2* DfbDrb + DfbDy2) + 
			(y2*pr3*((y1*(pr + aQrb)* DrbDy2)/ rbPy3bP2 + (a*cotb*DrbDy2)/
			rbP2 + (a*y1*DrbDy2)/ (rbPy3b* rbP2)))/ rbPy3b + 
...
/*		dg_ic[0][0][2] +=2.*(*/
        termPRL1=
			...
			DrbDy3b))/ (rbPz3bP2* rb);
        termPRL3=
            -(y2*y3bMa* (Dy3bDy3 + Dy3bDy3*DrbDy3b)* (-(cotb*pr3) + 
            (y1*(2.*pr + aQrb))/ rbPy3b + 
        ...
        termPRL5= 
			...
			DrbDy3b)/ rbP4;
        termPRL6= -(a*cosb*cotb*y2*pr3*Dy3bDy3* DrbDy3b)/
        ...
        dg_ic[0][0][2] +=2.*(termPRL1+termPRL3+termPRL4+termPRL5+termPRL6);
...
/*		dg_ic[0][1][0] +=2.*( */
        ...
        termPRL2=
			...
			DrbDy1)/ (rbPy3bP2* rb)))/ rbPy3b;
        termPRL3=(y3bMa*(((cosbP2*y2P2 - 
        ...
        termPRL4=
			...
			DrbDy1)/ (rbPz3b* rbP2);
        dg_ic[0][1][0] +=2.*(termPRL1+termPRL2+termPRL3+termPRL4); 
/*		dg_ic[0][1][0] +=
...
/*		dg_ic[0][1][2] +=2.*( */
        termPRL1=
            pr3*(((-pr + 2.*cotbP2*(1. - pr))* (Dy3bDy3 + Dy3bDy3*
        ...
        termPRL2=
			...
			rbPy3b;
        termPRL3=(y3bMa*(((cosbP2*y2P2 - (a*cotb*rbTcosbPy3b*
			...
			(rbPz3b* rbP2);
        dg_ic[0][1][2] +=2.*(termPRL1+termPRL2+termPRL3);
/*		dg_ic[0][1][2] +=
...
/*		dg_ic[1][0][0] +=2.*( */
        termPRL1= pr3*(-((cosb*(1. + 2.*cotbP2*(1. - pr))*
        ...
        termPRL2=(cotb*y3bMa*
			...
			rbPz3b;
        termPRL3=(pr3*(-(cotb*pr3) + (2.*y1*(pr + aQrb))/ rbPy3b + 
        ...
        dg_ic[1][0][0] +=2.*(termPRL1+termPRL2+termPRL3);
		...
		/* dg_ic[1][1][2] +=2.*( */
        termPRL1= -((y2*pr3*((y1*(pr + aQrb))/ rbPy3b - 
			cotb*(1. - 2.*pr - aQrb))* (Dy3bDy3 + Dy3bDy3* DrbDy3b))/
			...
			DrbDy3b)/ (rbPy3b* rbP2)))/ rbPy3b;
        termPRL2= -(y2*y3bMa* (Dy3bDy3 + 
        ...
        dg_ic[1][1][2] +=2.*(termPRL1+termPRL2+termPRL3);
		...
		/*dg_ic[1][2][1] +=2.*( */
        termPRL1=-2.*cotb*pr3*(1. - pr)*(DrbDy2/ rbPy3b - (cosb*DrbDy2)/
			...
			rb))/ rbPz3b;
        termPRL2= (y3bMa*((2.*pr*y1* DrbDy2)/ rbPy3bP2 + 
			...
			DrbDy2)/ (rbPz3b* rbP2);
        dg_ic[1][2][1] +=2.*(termPRL1+termPRL2); 
			
		/* dg_ic[1][2][2] +=2.*( */
        termPRL1= -2.*cotb*pr3*(1. - pr)*((Dy3bDy3 + Dy3bDy3* DrbDy3b)/
        ...
        termPRL2= 
			...
			(rbPz3b* rbP2);
        dg_ic[1][2][2] +=2.*(termPRL1+termPRL2);
	...
	--------------------------------------------------------------*/
#if 0 /* AMB. Don't symmetrize. We want displacement gradients. */
	for (i=0; i < 3; i++) {
		for (j=0; j < 3; j++) {
			for (k=0; k < 3; k++) {
				dummy[i][j][k] =
